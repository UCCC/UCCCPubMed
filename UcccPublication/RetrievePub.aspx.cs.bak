...
public partial class RetrievePub : System.Web.UI.Page
{
    ...
    protected void GetAndInsertPubForOneMember(string clientPlusSign)
    {
        ...
        foreach (int pmid in pmidListFromPubMed)
        {
            ...
            {
                GetAndInsertAllInfoForOnePmid(pmid,false);
            }
        }
    }
    protected List<int> GetPmidListForOneMemberInDb(string clientWithPlusSign)
    {
        ...
        try
        {
            pmidIdList = (from IDataRecord record in myReader
                                       select (int)record["pmid"]).ToList();
        }
        finally
        {
            myReader.Close();
    ...
    protected List<int> GetPmidListForOneMemberFromPubMed(string clientWithPlusSign)
    {
        ...
        {
            url = @"http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=" + clientWithPlusSign + @"[au]" + "&retmax=5000&usehistory=y";
        }
        else
        {
    ...
    protected void btnGetPubByProgram_Click(object sender, EventArgs e)
    {
        ...
        SavePublog(ddlProgram.SelectedItem.ToString(), DateTime.Now);
    ...
    protected int GetAndInsertAllInfoForOnePmid(int pmid, bool forceImport)
    {
        ...
        //if (MemberConflict(authorList))
        {
            ...
            SavePublog(pmid.ToString() + " name conflict", today);
            if (!forceImport)
            {
            ...
            string pubmedYearStr, string pubmedMonthStr, string pubmedDayStr)
    {
        ...
        {
            SavePublog(pmid.ToString() + ", " + ex.Message.ToString() , today);
        }
        finally
        {
            myConnection.Close();
        ...
        SavePublog(pmid.ToString(), today);
    ...
    protected void btnImportBySpecifiedName_Click(object sender, EventArgs e)
    {
        ...
        string url = @"http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=" + clientPlusSign + @"[au]" + "&retmax=5000&usehistory=y";
        SearchPub(url);
